...
static int fsa = 0;
struct udevice *fsa_gpiodevs[FSA_MAX] = { NULL };
...
static const char *fsa_gpio_config_names[] = { "NC", "", "input", "output-low", "output-high" };
...
static void fsa_show_gpio_descs(const char *prefix, int fsa, struct fsa_board_info *board_info, struct fsa_user_info *user_info)
{
	char str[128];
	int i;
	...
	if (fsa_gpiodevs[fsa]) {
		for (i = board_info->sockgpios; i < (board_info->sockgpios + board_info->ioexpgpios); i++) {
			fsa_get_gpio_desc(&user_info->gpios[i], str, sizeof(str));
			printf("%s%-2d: %s\n", prefix, i, str);
		}
	}
...
static int ft_fixup_stringlist_elem(void *fdt, int offset, const char *prop, int elem, const char *val)
{
	const char *list, *end;
	char *new, *buf;
	int length;
	...
	for (fsa = 1; fsa < FSA_MAX; fsa++) {
		...
		for (i = 0; i < board_info.sockgpios + board_info.ioexpgpios; i++) {
			...
			if (user_info.gpios[i].source == 0xff) {
				for (off = fdt_node_offset_by_prop_value(fdt, 0, "gpio-controller", NULL, 0);
				     off >= 0;
				     off = fdt_node_offset_by_prop_value(fdt, off, "gpio-controller", NULL, 0))
				{
					sprintf(str, "fsa%d_gpio%d", fsa, user_info.gpios[i].offset + 1);
					ret = fdt_stringlist_search(fdt, off, "gpio-line-names", str);
					if (ret >= 0) {
						ft_fixup_fsa_gpio_name(fdt, off, fsa, ret, user_info.gpios[i].name);
						break;
					}
				}
			}
			...
			/* port expander GPIOs */
				...
				off = fdt_subnode_offset(fdt, fsa_get_dtnode(fdt, fsa), "gpio@20");
				ft_fixup_fsa_gpio_name(fdt, off, fsa, user_info.gpios[i].offset, user_info.gpios[i].name);
			}
		}
	}
...
static int do_fsa_desc(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	/* set */
	if (argc) {
		strncpy(user_info.desc, argv[0], sizeof(user_info.desc));
...
static int do_fsa_overlay(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	/* set */
	if (argc) {
		strncpy(user_info.overlay, argv[0], sizeof(user_info.overlay));
		if (fsa_write_user_config(fsa, &user_info))
			return CMD_RET_FAILURE;
	}
...
static int do_fsa_gpio(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if (i >= board_info.sockgpios && i < (board_info.sockgpios + board_info.ioexpgpios) && fsa_gpiodevs[fsa]) {
